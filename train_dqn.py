...
def main(env_name, tags=None, load_dir=None, fresh_state=False, rnd_seed=23,
    ...
    learning_rate=1e-4, batch_size=32, train_freq=4, target_update_freq=1000,
    hiddens=[512], double_q=True, dueling_q=True, prioritized_replay=True,
    pr_alpha=0.6, pr_beta0=0.4, pr_epsilon=1e-6, param_noise=False,
    layer_norm=False, pn_update_freq=50, pn_reset_freq=10000):
    ...
    Args:
        ...
        load_dir (str): Absolute path to model director containing tf checkpoint.
        fresh_state (bool): Reset replay buffer and training step.
        rnd_seed (int): Random seed for environment and session.
        total_steps (int): Number of iterations to train.
        save_freq (int): Save model and state every n steps.
    ...
    Returns:
        float: The highest average 100 episode reward encountered.
    ...
    with U.make_session(4) as sess:
        ...
        for step in range(total_steps):
            ...
            if param_noise:
                ...
                update_eps = exploration.value(round(np.random.normal(epsilon_step, 100)))

            # Get Transition
            reset = False
            action = act(np.array(obs)[None], update_eps=update_eps, **kwargs)[0]
            new_obs, rew, done, _ = env.step(action)
            if step % buffer_append_freq == 0:
            ...
            # Train the Network
            if step > min(100000, buffer_size) and step % train_freq == 0:
                # Sample Transitions
                if prioritized_replay:
            ...
            improved = saved_reward is None or np.mean(episode_rewards[-100:]) > saved_reward
            if step > 0 and (step % save_freq == 0 or step > total_steps) and improved:
                saved_reward = np.mean(episode_rewards[-100:])
                save_model(save_dir, {'replay_buffer': replay_buffer, 'step': step}, saved_reward)
                logger.log('\nReward of saved model: ' + str(saved_reward) + '\n')
...
def save_model(save_dir, state, reward):
    try:
        model_dir = 'model-{}-{}'.format(state['step'], reward)
...
def load_model(save_dir):
    try:
        ...
        if os.path.exists(state_path):
            ...
            model_dir = 'model-{}'.format(state['step'])
...
if __name__ == '__main__':
    main(
        ...
        tags=['10x10', '1 random cancer'],
        total_steps=1000000,
        save_freq=10000,
        buffer_size=100000,
        buffer_append_freq=1,
        epsilon_sched=[(0, 1.0), (400000, 0.6), (800000, 0.01)],
        learning_rate=1e-4,
        batch_size=32,
        train_freq=4,
