...
model_path = os.path.join(PROJ_DIR, 'models', 'deepq', 'pong_*.pkl')
try:
    ...
    if ('done' in locals and locals['done'] == True):
        if ('mean_100ep_reward' in locals and locals['num_episodes'] >= 1
            ...
            filename = os.path.join(PROJ_DIR,
                'models/deepq/pong_%s.pkl' % locals['mean_100ep_reward'])
            act.save(filename)
...
def main():
    ...
    parser.add_argument('--load_state', type=str, default='pong_model.pkl')
    ...
    act.save(os.path.join(PROJ_DIR, 'models', 'deepq', 'pong_final.pkl'))
