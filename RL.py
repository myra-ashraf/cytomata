...
def trainGraph(inp, out):
    ...
    while(1):
        ...
        if(random.random() <= epsilon and not USE_MODEL):
            ...
            maxIndex = choice((0,1,2), 1, p=(0.90, 0.05,0.05))[0]
        else:
        ...
        frame, reward_t, terminal, _ = game.step(maxIndex)
        ...
        dt = round(t1 - t0, 4)
        t0 = t1
        print("\rTIMESTEP: {} | LOOP_SECONDS: {:.4f} | EPSILON: {:.2f} | REWARD: {} | Q_MAX: {:.4f}".format(
            t, dt, epsilon, reward_t, np.max(out_t)), end="")
        sys.stdout.flush()
